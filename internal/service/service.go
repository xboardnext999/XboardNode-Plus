...
const (
	controlPlanePollAttempts       = 3
	controlPlanePollRetryDelay     = 500 * time.Millisecond
...
func newService(cfg *config.Config, cp controlplane.ControlPlane) *Service {
	...
	kernelMode := strings.ToLower(strings.TrimSpace(cfg.Kernel.Type))
	if kernelMode == "" {
		kernelMode = model.KernelAuto
	}
	initialKernelType := kernelMode
	if initialKernelType == model.KernelAuto {
		initialKernelType = model.KernelSingBox
	}
	...
	svc.configureKernelCallbacks()
	return svc
}

func newKernel(kcfg config.KernelConfig, kernelType string) kernel.Kernel {
	...
	default:
		...
		kcfg.Type = model.KernelSingBox
		return singbox.New(kcfg)
	}
}

func protocolsForKernelType(kernelType string) []string {
	return newKernel(config.KernelConfig{Type: kernelType}, kernelType).Protocols()
}

func (s *Service) configureKernelCallbacks() {
	if s.kernel == nil {
		return
	}
...
func (s *Service) resolveKernelType(nc *model.NodeSpec) (string, error) {
	return model.ResolveEffectiveKernelType(nc, s.kernelMode)
}

func (s *Service) validateConfigForKernel(nc *model.NodeSpec) (string, error) {
	kernelType, err := s.resolveKernelType(nc)
	if err != nil {
		return "", err
	}
	if err := validateNodeRuntime(kernelType, protocolsForKernelType(kernelType), nc, s.cert.TLSCert()); err != nil {
		return "", err
	}
	return kernelType, nil
}

func (s *Service) ensureKernelType(kernelType string, nc *model.NodeSpec) {
	if kernelType == "" {
		kernelType = model.KernelSingBox
	}
	if s.kernel != nil && s.kernelType == kernelType {
		return
	}

	previous := s.kernelType
	if s.kernel != nil {
		s.kernel.Stop()
	}
	s.kernel = newKernel(s.cfg.Kernel, kernelType)
	s.kernelType = kernelType
	s.configureKernelCallbacks()

	logger := nlog.Core()
	if s.nodeLog != nil {
		logger = s.nodeLog
	}
	if s.kernelMode == model.KernelAuto {
		logger.Info("kernel auto selected",
			"configured_kernel", s.kernelMode,
			"previous_kernel", previous,
			"effective_kernel", kernelType,
			"protocol", nodeProtocol(nc),
			"transport", nodeTransport(nc),
		)
	} else {
		logger.Info("kernel switched",
			"configured_kernel", s.kernelMode,
			"previous_kernel", previous,
			"effective_kernel", kernelType,
			"protocol", nodeProtocol(nc),
...
func nodeProtocol(nc *model.NodeSpec) string {
	if nc == nil {
		return ""
	}
	return nc.Protocol
...
func nodeTransport(nc *model.NodeSpec) string {
	if nc == nil {
		return ""
	}
	return nc.Network
...
func (s *Service) initialSetup(ctx context.Context) error {
	...
	s.ensureKernelType(kernelType, bootstrap.Config)
...
func (s *Service) handleWSEvent(ctx context.Context, event controlplane.Event) {
	...
	case controlplane.EventSyncConfig:
		...
		kernelType, err := s.validateConfigForKernel(event.Config)
		if err != nil {
			nlog.Core().Warn("ws config validation failed, ignoring update", "error", err)
			return
		}
		s.ensureKernelType(kernelType, event.Config)
...
func (s *Service) pullViaAPIAsync(ctx context.Context) {
	...
	go func() {
		...
		snapshot, err := s.pollControlPlaneWithRetry(ctx)
		if err != nil {
...
func (s *Service) pollControlPlaneWithRetry(ctx context.Context) (controlplane.Snapshot, error) {
	var lastErr error
	for attempt := 1; attempt <= controlPlanePollAttempts; attempt++ {
		snapshot, err := s.source.Poll(ctx)
		if err == nil {
			return snapshot, nil
		}
		lastErr = err
		if attempt == controlPlanePollAttempts {
			break
		}
		nlog.Core().Warn("poll control plane failed, retrying",
			"attempt", attempt,
			"max_attempts", controlPlanePollAttempts,
			"error", err,
		)
		select {
		case <-time.After(time.Duration(attempt) * controlPlanePollRetryDelay):
		case <-ctx.Done():
			return controlplane.Snapshot{}, ctx.Err()
		}
	}
	return controlplane.Snapshot{}, lastErr
}
...
func (s *Service) applyPullResult(ctx context.Context, result pullResult) {
	...
	configChanged := false
	restUsersChanged := false
	var restDiag *syncDiagnostics

	if result.users != nil {
		diag, ok := s.beginUserSync("rest", result.users)
		if !ok {
			s.finishUserSync(diag, "skipped", "skipped")
			result.users = nil
			result.userHash = ""
		} else {
			restDiag = &diag
	...
	if result.config != nil {
		kernelType, err := s.validateConfigForKernel(result.config)
		if err != nil {
			nlog.Core().Warn("runtime config validation failed", "error", err)
			result.config = nil
		} else {
			s.ensureKernelType(kernelType, result.config)
			configChanged = true
	...
	if result.users != nil {
		...
		restUsersChanged = usersChanged

		if usersChanged && !configChanged {
			if restDiag != nil {
				s.applyUserUpdateWithDiagnostics(ctx, result.users, result.userHash, *restDiag)
				restDiag = nil
			} else {
	...
	if restDiag != nil {
		inboundStatus := "unchanged"
		if restUsersChanged {
			inboundStatus = "skipped_config_reload"
		}
		s.finishUserSync(*restDiag, inboundStatus, reloadStatus)
	}
}
...
func (s *Service) beginUserSync(source string, users []model.UserSpec) (syncDiagnostics, bool) {
	if users == nil {
		users = []model.UserSpec{}
	}
	...
	toAdd, toRemove := kernel.UserDiff(prevUsers, users)
	diag := syncDiagnostics{
		source:        source,
		previousUsers: len(prevUsers),
	...
	s.syncLogger().Info("sync start",
		"source", source,
		"previous_users", diag.previousUsers,
		"fetched_users", diag.fetchedUsers,
		"added", diag.added,
		"removed", diag.removed,
	)

	if reason := rejectFetchedUsers(prevUsers, users); reason != "" {
		s.syncLogger().Error("sync aborted, keeping previous users",
			"source", source,
			"previous_users", diag.previousUsers,
			"fetched_users", diag.fetchedUsers,
			"added", diag.added,
...
func (s *Service) finishUserSync(diag syncDiagnostics, inboundStatus, reloadStatus string) {
	reloadXray := "not_xray"
	if s.kernel != nil && s.kernel.Name() == "xray" {
		reloadXray = reloadStatus
	}
...
func rejectFetchedUsers(previous, fetched []model.UserSpec) string {
	if len(previous) > 0 && len(fetched) == 0 {
		return "empty user snapshot"
	}

	seenIDs := make(map[int]struct{}, len(fetched))
	seenUUIDs := make(map[string]struct{}, len(fetched))
	for i, user := range fetched {
		if user.ID <= 0 {
			return fmt.Sprintf("invalid user id at index %d", i)
		}
		uuid := strings.TrimSpace(user.UUID)
		if uuid == "" {
			return fmt.Sprintf("empty user uuid at index %d", i)
		}
		if _, ok := seenIDs[user.ID]; ok {
			return fmt.Sprintf("duplicate user id %d", user.ID)
		}
		if _, ok := seenUUIDs[uuid]; ok {
			return fmt.Sprintf("duplicate user uuid %q", uuid)
		}
		seenIDs[user.ID] = struct{}{}
		seenUUIDs[uuid] = struct{}{}
	}

	if len(previous) >= abnormalUserDropMinPrevious &&
		len(fetched)*abnormalUserDropRatioDenom < len(previous)*abnormalUserDropRatioNumerator {
		return fmt.Sprintf("abnormal user count drop: previous=%d fetched=%d", len(previous), len(fetched))
	}
	return ""
}
...
func (s *Service) startKernel(nc *model.NodeSpec, users []model.UserSpec) bool {
	kernelType, err := s.resolveKernelType(nc)
	if err != nil {
		nlog.Core().Error("failed to resolve kernel", "error", err)
		return false
	}
	s.ensureKernelType(kernelType, nc)
...
func (s *Service) applyUserUpdate(ctx context.Context, users []model.UserSpec, newHash, source string) {
	diag, ok := s.beginUserSync(source, users)
	if !ok {
		s.finishUserSync(diag, "skipped", "skipped")
		return
	}
	s.applyUserUpdateWithDiagnostics(ctx, users, newHash, diag)
}

func (s *Service) applyUserUpdateWithDiagnostics(ctx context.Context, users []model.UserSpec, newHash string, diag syncDiagnostics) {
	if !s.ensureRunning() {
		s.finishUserSync(diag, "skipped_not_running", "skipped")
		return
	}
	...
	if err != nil {
		...
		s.finishUserSync(diag, "failed", reloadStatus)
		return
	}
	if newHash != "" {
		s.lastUserHash = newHash
	...
	s.finishUserSync(diag, "success", "skipped")
}
...
func (s *Service) applyChanges(ctx context.Context, configChanged, usersChanged bool) string {
	...
	kernelType, err := s.resolveKernelType(s.lastConfig)
	if err != nil {
		nlog.Core().Warn("kernel resolution failed", "error", err)
		return "failed"
	}
	s.ensureKernelType(kernelType, s.lastConfig)
...
func (s *Service) pushReportAsync() {
	...
	go func() {
		...
		if err := s.sink.Report(controlplane.ReportPayload{Traffic: traffic, Alive: aliveIPs, Online: online, Access: access, CPU: status.CPU, Mem: [2]uint64{status.MemTotal, status.MemUsed}, Swap: [2]uint64{status.SwapTotal, status.SwapUsed}, Disk: [2]uint64{status.DiskTotal, status.DiskUsed}, Metrics: metrics}); err != nil {
